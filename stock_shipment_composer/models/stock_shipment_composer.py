...
class StockShipmentComposer(models.Model):
    _inherit = ["mail.thread", "mail.activity.mixin", "stock.actual.date.mixin"]
    ...
    amount_declared_total = fields.Monetary(
        compute="_compute_amount_total",
        string="Declared Total Amount",
    )
    ...
    def _compute_currency_id(self):
        for rec in self:
            currency = rec.line_ids[:1].move_id.sale_line_id.currency_id
            rec.currency_id = currency or rec.company_id.currency_id

    @api.depends("line_ids.amount_subtotal", "line_ids.amount_declared")
    ...
    def _compute_note(self):
        for rec in self:
            rec.note = rec.line_ids.move_id.sale_line_id.order_id[:1].note

    @api.depends("line_ids.move_id", "line_ids.move_id.state")
    def _compute_move_ids(self):
        for rec in self:
            rec.move_ids = rec.line_ids.move_id
            rec.sale_line_ids = rec.move_ids.sale_line_id
    ...
    def write(self, vals):
        """Propagate `actual_date` to pickings in case the state is done."""
        res = super().write(vals)
        for rec in self:
            if "actual_date" in vals and rec.state == "done":
                rec.move_ids.picking_id.actual_date = rec.actual_date
        return res
